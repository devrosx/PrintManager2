...
struct CompactPreviewPanel: View {
    ...
    private func openInlineQuickLook(for file: FileItem) {
        withAnimation(.easeInOut(duration: 0.2)) {
            appState.quickLookFileIDs = appState.files.map(\.id)
            appState.quickLookFileID  = file.id
            appState.quickLookCurrentPage = currentPage
            appState.quickLookMode = file.fileType.isImage ? .singlePage : .thumbnails
            appState.showInlineQuickLook = true
        }
    }
                ...
                if let file = selectedFile {
                    VStack(spacing: DS.Spacing.small) {
                        PreviewImageView(file: file, currentPage: $currentPage)
                            ...
                            .onTapGesture { openInlineQuickLook(for: file) }
